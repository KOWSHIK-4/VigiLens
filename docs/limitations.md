...
- The AI service's machine-to-machine endpoints (`/capture`, `/detect/image`,
  `/detect/video`, `/detect/detectors`, `/detect/webcam`,
  ...
  set `AI_REQUIRE_AUTH=true` to force the check in development too. The
  legacy alias `AI_STATS_REQUIRE_AUTH` is still honored but
  `AI_REQUIRE_AUTH` is the canonical name.
